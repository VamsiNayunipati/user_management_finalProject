...
@pytest.mark.asyncio
async def test_update_user_email_access_allowed_if_already_exists(async_client, admin_user, verified_user, admin_token):
    updated_data = {"email": f"updated_{admin_user.id}@example.com"}
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = await async_client.put(f"/users/{admin_user.id}", json=updated_data, headers=headers)
    response = await async_client.put(f"/users/{verified_user.id}", json=updated_data, headers=headers)
...
@pytest.mark.asyncio
async def test_update_user_email_access_allowed_with_admin_user(async_client, admin_user, verified_user, admin_token):
    updated_data = {"email": f"updated_{admin_user.id}@example.com"}
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = await async_client.put(f"/users/{admin_user.id}", json=updated_data, headers=headers)
    response = await async_client.put(f"/users/{admin_user.id}", json=updated_data, headers=headers)
...
@pytest.mark.asyncio
async def test_update_linkedin_url_null(async_client, admin_user, admin_token):
    updated_data = {
        "email": f"updated_{admin_user.id}@example.com",
    ...
    response = await async_client.put(f"/users/{admin_user.id}", json=updated_data, headers=headers)
    assert response.status_code == 200
    assert response.json()["linkedin_profile_url"] == None

@pytest.mark.asyncio
async def test_update_profile_pic_url_null(async_client, admin_user, admin_token):
    updated_data = {
        "email": f"updated_{admin_user.id}@example.com",
        "profile_picture_url": ""
        }
...
@pytest.mark.asyncio
async def test_update_to_professional_status_is_Allowed(admin_token, admin_user, async_client):
    headers = {"Authorization": f"Bearer {admin_token}"}
    response = await async_client.patch(f"/update/{admin_user.id}/user",  headers=headers)
    assert response.status_code == 200
    assert response.json()["is_professional"] == True

@pytest.mark.asyncio
async def test_update_to_professional_status_is_not_Allowed(admin_user, user_token, async_client):
    headers = {"Authorization": f"Bearer {user_token}"}
    response = await async_client.patch(f"/update/{admin_user.id}/user",  headers=headers)
    assert response.status_code == 403
