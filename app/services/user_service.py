...
class UserService:
    ...
    @classmethod
    async def upgrade_user_to_professional(cls, session: AsyncSession, user_id: UUID, email_service: EmailService):
        user = await cls.get_by_id(session, user_id)
        email_flag = True
        ...
        if user:
            ...
            await session.commit()
            if email_flag == True:
                await email_service.send_email_regarding_professional_update(user.email)
            return user
        return None
